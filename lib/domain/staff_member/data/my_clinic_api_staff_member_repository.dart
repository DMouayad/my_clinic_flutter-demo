...
class ApiStaffMemberRepository extends BaseStaffMemberRepository {
  ApiStaffMemberRepository() {
    _dataSource = const ApiStaffMemberDataSource();
  }

  late final ApiStaffMemberDataSource _dataSource;
    ...
    return await (await _dataSource.addStaffMember(email, userRole))
      ...
      onSuccess: (newStaffMember) async {
        if (staffMembersResource != null) {
          List<ApiStaffMember> staffMemberList = List.from(
  ...
  Future<Result<VoidValue, AppError>> deleteStaffMember(int id) async {
    return (await _dataSource.deleteStaffMember(id)).mapSuccessToVoid(
        onSuccess: (_) {
      final List<ApiStaffMember> currentStaffMembers =
